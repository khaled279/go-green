...
public interface ProductRepository extends JpaRepository<ProductEntity, Long>,
	...
	@Query("SELECT p FROM ProductEntity p " + "WHERE (:productName IS NULL OR p.name LIKE %:productName%) " + "AND (:maximumPrice IS NULL OR p.price <= :maximumPrice) AND (:minimumPrice IS NULL OR p.price >= :minimumPrice)" + "AND (COALESCE(:productIds, NULL) IS NULL OR p.id IN (:productIds)) AND ( :vendorId IS NULL OR p.vendor.id = :vendorId) " + "ORDER BY p.viewsCount ")
